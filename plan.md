...
3. **表・一覧・散布図ラベルの日本語化は 1・2 の後で検討**（ふくたろう指示）。論点: 幅（「シェイ・ギルジャス＝アレクサンダー」はスマホで切れる）、散布図は姓だけ、同名の識別（ジェイレン／ジェイリン・ウィリアムズ）は英語副表記で担保
   ...
   - ✅（2026-09-04 後半実装）スマホ実機確認で「行けそう」（ふくたろう）となり、選手一覧の表・チーム詳細ロスター（RS/PO）・ボール支配の帯・ワンマン度の説明文も短縮名に。選手一覧は検索照合が英語名フィールドを使うため、表示専用マップ `namesShort`（`playerId-team` → 短縮名）を渡す方式。**残り**: 試合詳細ボックススコアの名前列のみ未対応
