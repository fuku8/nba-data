...
新シーズンへ向けたトップページ（§13-3）等の残課題と過去季対応は**一括して進める**。方針は次の2点で、これが nba-data の過去季設計の正（§12-3〜12-5 と整合。矛盾したらこの節が勝つ）:
...
- 過去季の選手個別ページ（`/players/[id]/2025-26`）を残すか、積み重ね表に統合して廃止するか。廃止なら §12-11 の Cloudflare ファイル数上限（過去季3つ目で 20,000 超）の懸念も同時に消える。残すなら、過去季ページ内リンクが全て当季へ出る点（2026-09-22 コード実測: 季を保つ導線は SeasonSwitch のみ）を「独立アーカイブ」の枠内で再設計する
