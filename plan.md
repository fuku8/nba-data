...
3. **過去季の選手個別ページ（`/players/[id]/2025-26`）は残す**（2026-09-22 ふくたろう決定・追補）: 選手個別の feel-viz が nba-data のメイン部分のため、積み重ね表への統合・廃止はしない。残す前提で**内容を圧縮する方法を検討する**——§12-11 の Cloudflare ファイル数上限（過去季3つ目・2029年春で 20,000 超）への対策。既出の案 (a) 過去季ページをローテ選手（GP≥MIN_GP）に絞る (b) 最古の季を落とす に加えて、1ページあたりの出力削減（実測: HTML 349KB/人＋RSC `.txt`＋セグメント計約9ファイル/人）の圧縮余地を着手時に調査する
...
- 過去季の選手個別ページの圧縮方式（上記3の選択肢のどれを採るか）
- 過去季ページ内リンクが全て当季へ出る点（2026-09-22 コード実測: 季を保つ導線は SeasonSwitch のみ）を「独立アーカイブ」の枠内でどう再設計するか
