...
1. ✅（2026-09-03 実装）対応表 → title・description・比較ページ検索（カタカナでも英字でも当たる）に使う。表示は変えない。検索流入とサイト内検索の利得はここで取れる → 検証: 本番 HTML の `<title>` に日本語名、比較ページで「ヨキッチ」入力が当たる
   - 実装: `lib/data/names-ja.ts`（対応表の読み込み）。選手ページ title「ニコラ・ヨキッチ（Nikola Jokić） · NBA 2025-26」、チーム詳細も同型。比較検索は `playerJa` を照合に追加し、`normalizeName` にひらがな→カタカナ変換を足したので「よきっち」でも当たる（実測: ヨキッチ・よきっち・ドンチッチ・jokic・ウェンバンヤマ全て候補表示）。選手一覧ページの検索は英語名照合のまま（日本語対応はヘッダー検索 §13-3 C で統一予定）
2. 選手ページ見出しを日本語名＋英語名（副）、OG 画像も同じ → 検証: `/players/203999` と OG の実表示
