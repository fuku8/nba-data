...
- **リーダーの図（RS/PO 共通）**: 対象＝そのリストの上位 20 人の和集合。「図に載る人＝リストに載っている人」で §11 の「図はリストのスタッツだけで構成」の延長。守備=STL∪BLK（RS40・PO36 点）、USG%×TS%（RS40・PO40 点）。全員がどちらかの軸の上位なので左下象限はほぼ空く（象限ラベルの文言は実画面を見て要調整かも）
