...
| 案 | 効く観点 | 内容 | 目安 |
|---|---|---|---|
| A 今日の一枚 | 来る・戻る | 1 選手の Percentile バー（8 行）を日替わりで大きく置く。選び方は決定論（日付ハッシュ × GP 上位 100 人）。オフでも昨季データで回る。サイトの看板図をトップに常設する最小形 | 半日 |
| B タイプ別 1 位の帯 | 来る | 7 タイプの RS 1 位（PO 中は PO）を横スクロールのカード帯で。選手名（日本語）＋タイプ名＋評価点。`/types` への入口 | 半日 |
| C 図表カタログ（§12-6 既記） | 来る | `/metrics` に実データの図を 1 枚ずつ貼り、ナビ名を「図表」に。初見が「何があるサイトか」を 1 ページで掴む | 1 日 |
| D 順位表の Heartbeat 列（§12-6 既記） | 来る | 順位表 Top8×2 に直近 10 試合の点差バー | 半日 |
| E 見どころ 3 枚（§12-6 既記） | 戻る | 事件系優先・分析系プール。開幕後に価値が出る | §12-8 Phase 2 |

**推奨順**: 開幕前に A・D・C（試合データが無くても昨季で検証できる）、開幕時に E、B は A と入口が重なるので A の反応を見てから。トップの順位表とリーダー 3 枚は残す（順位表は D で署名化、リーダーは日本語名化で読みやすくなる）。
