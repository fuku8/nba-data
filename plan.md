...
## 13. 第三者評価（2026-09-02）からの改善プラン

出典: `~/fukuta_ob/_ai_workspace/reports/2026-09-02-nba-data-第三者評価.md`（Claude の評価＋Codex 独立評価の照合）。ふくたろうの採択（同日）に基づく。

### 13-1. 選手名・チーム名の日本語化

**方針**: 日本語を主、英語を従の併記。置き換えではない。読めない英字（Jokić・Dončić・Krejčí）は看板選手にもあり、対象読者が日本なので表示も日本語にする。
...
1. ✅（2026-09-03 実装）対応表 → title・description・比較ページ検索（カタカナでも英字でも当たる）に使う。表示は変えない。検索流入とサイト内検索の利得はここで取れる → 検証: 本番 HTML の `<title>` に日本語名、比較ページで「ヨキッチ」入力が当たる
   - 実装: `lib/data/names-ja.ts`（対応表の読み込み）。選手ページ title「ニコラ・ヨキッチ（Nikola Jokić） · NBA 2025-26」、チーム詳細も同型。比較検索は `playerJa` を照合に追加し、`normalizeName` にひらがな→カタカナ変換を足したので「よきっち」でも当たる（実測: ヨキッチ・よきっち・ドンチッチ・jokic・ウェンバンヤマ全て候補表示）。選手一覧ページの検索も同日対応（namesJa を渡してカタカナ・ひらがなで表を絞り込み）
2. ✅（2026-09-03 実装）選手ページ見出しを日本語名＋英語名（副）、OG 画像も同じ。**比較ページの選択バッジ・凡例・表の列ヘッダーもここで日本語化**（プリセットは日本語・結果は英語という段階1の不一致を解消。同時表示4人までなので幅の論点が小さく先行しやすい。2026-09-03 ふくたろう了承） → 検証: `/players/203999` と OG の実表示
   - 実装: 見出し h1「ニコラ・ヨキッチ」＋副記に英語名。OG は Noto Sans JP を対応表の全文字＋ASCII の 203 字にサブセット化（各 55KB・`scripts/subset-og-font.py`、繰越の対応表更新後に再実行）してローカル同梱し、タイトルを日本語名に。全角は幅換算 2 でタイトルサイズを段階縮小（最長のシェイ・ギルジャス＝アレクサンダーで 52px に収まることを実画像で確認、漢字の楊瀚森も描画確認）。ビルドは 650 枚込みで 17 秒・安定。**2026-09-02 の「OGP は英数字のみ」決定は前提（画像ごとのフォント取得）が消えたため更新**。比較ページは dn()（playerJa ?? player）でバッジ・凡例・レーダー・表・ワッフル・候補を日本語表示（候補は英語名を副記）
3. **表・一覧・散布図ラベルの日本語化は 1・2 の後で検討**（ふくたろう指示）。論点: 幅（「シェイ・ギルジャス＝アレクサンダー」はスマホで切れる）、散布図は姓だけ、同名の識別（ジェイレン／ジェイリン・ウィリアムズ）は英語副表記で担保
   - ✅（2026-09-04 前半実装）**短縮表示名の方式**（ふくたろう了承。「略称表記のサイトはあまりないのでちょっとした特徴にもなりそう」）: 既定はカタカナ姓の自動生成（「・」末尾。ジュニア・n世は前と結合、「＝」複合姓は保持）、日本の記事で定着した頭文字略称だけ手動上書き（names-ja.ts の `PLAYER_NAME_SHORT`: SGA・KCP・NAW・MPJ）。同チームに同姓がいる場合（兄弟・父子など実測8組: アデトクンボ3兄弟・ジェームズ父子・カリー兄弟・バグナー兄弟等）は日本語フル名に自動フォールバック（`withDisplayNames`）。**名前の機械的切り詰め（truncate）は原則使わない**（「ケンテイビアス・コール…」は判別不能のため。切るなら姓か略称を出す）
   - 適用箇所: リーダーズ（リーダー表・散布図2枚のラベル）、トップ/POトップのスタッツリーダー、選手一覧の散布図2枚のラベル（表は英語のまま）。quadrant-map のラベル幅概算を全角対応（全角=半角2倍）
   - ✅（2026-09-04 後半実装）スマホ実機確認で「行けそう」（ふくたろう）となり、選手一覧の表・チーム詳細ロスター（RS/PO）・ボール支配の帯・ワンマン度の説明文も短縮名に。選手一覧は検索照合が英語名フィールドを使うため、表示専用マップ `namesShort`（`playerId-team` → 短縮名）を渡す方式。**残り**: なし（段階3完了）
   - ✅（2026-09-04）ボックススコアも短縮名＋名前列固定（sticky left-0。列13本の横スクロールで誰の行か見えるように。ふくたろう指摘）。NAW の略称は「ニキール」に変更（日本ではファーストネームで流通・リーグに1人。「NAW」「ニキール・A」は流通していない表記のため不採用）
   - 🔄（2026-09-06 方針転換・ふくたろう指摘）**表・リスト・文中はフル日本語名、短縮名（SGA・カタカナ姓）は図内ラベル（散布図・ボール支配の帯）限定に変更**。理由: (a) 短縮名の3層構造は同一選手の表記が文脈（同チームの同姓有無）で揺れ、ロスターが動く新シーズンに悪化する (b) SGA/MPJ・姓主体の表記は「フルネームで記憶している」ファンに分かりにくい (c) スマホの切り詰め「シェイ・ギルジャ...」「ジェイレン・ブラウ...」は判別不能。スマホは nba.com と同型の折り返し（名前列 min-w 6.5em・max-w 120px・2行）で切り詰めない。実装: `withFullNames` を新設し表系の `withDisplayNames`/`namesShort` を置換（選手一覧・リーダー表・トップ/POトップ・チーム詳細ロスターRS/PO・ワンマン度説明文・ボックススコア・タイプページ）。`withDisplayNames` は図ラベル専用として存続
   - 同時修正: ①タイプページ（/types）が日本語化から漏れて英語名のままだった ②選手一覧の namesShort が perGame+advanced 連結後に同姓判定していたため全員が「同チーム同姓」誤判定でフル名フォールバックしていた（表記が仕様と違った実因。namesShort 廃止で解消） ③ボックススコアの名前列が w-full の表で min-content（2文字）まで潰れて折り返していた → min-w 付与 ④ボックススコアのチーム名帯が横スクロールに追従して消えていた → スクロールの器の外へ ⑤比較表・RSロスターに名前列固定（sticky）が無かった → 他の表と統一
   - 🔄（2026-09-06 スマホ実機確認のふくたろう指摘で調整）名前列が狭く4文字程度で改行され窮屈 → 幅を min/max 12em（比較表は色丸ぶん 14em）に拡大し、`SegmentedName`（「・」「＝」区切りを whitespace-nowrap の span で包む）で**姓名の途中で折らない**改行に（nba.com の姓名改行と同型）。Chromium 実測でヨキッチ1行・SGA/KCP/MPJ 2行。あわせて比較表のフォントを他の表と同じ text-xs sm:text-sm に（スマホだけ大きかった）、比較表の注記をスクロールの器の外へ
   - レビュー（2026-09-06 codex-reviewer＝OpenAI系・コミット acfaa1c）指摘2件: ①「ボックススコアの外枠 overflow-hidden が内側 overflow-x-auto の sticky を壊す」→ **却下**（sticky の基準は最も近いスクロール祖先＝内側の overflow-x-auto。Chromium 実測で 169px スクロール後もセル X 座標不変。Safari は未確認・スマホ実機確認時に見る） ②タイプページの選手名リンクに truncate が残存 → 妥当、折り返しに修正
   - 不具合修正: クリッパーズだけ英語のまま → `team_names_ja.csv` の TEAM_ABBREVIATION 欄が空（NBA.com がクリッパーズだけ略称欄空・"LA Clippers" 表記で返す既知の癖が生成スクリプト経由で伝播）。CSV に LAC を補い、`fetch-player-names-ja.py` にも再生成で戻らないよう補正を追加
4. ✅（2026-09-04 実装）チーム名は検索に当たる形（オクラホマシティ・サンダー）で順位表の正式名に。略称（OKC）はそのまま
   - 実装: 正式名を画面表示する全6箇所 — 順位表・トップの順位表・チーム一覧（RS/PO）・チーム詳細 h1（日本語主＋英語副、選手ページと同型）・選手ページのチームリンク。略称（スマホ表示・OKC等）は不変。ビルド後の出力 HTML で日本語名・英語名の出現数を実測して確認（teams.html は英語正式名 0 件）

### 13-2. 小さい修正（実施済み 2026-09-03）

7 件すべて実装した。#1 は固定リンクでなくボタン（`setSelectedIds`）にした — `?ids=` リンクだと同一ページ内遷移で `useState` 初期化が走らないため。両選手がデータに居る組だけ表示する。#4 は `/metrics` 冒頭の「このサイトについて」カード（`#about`）＋全ページ共通フッター 1 行。sitemap は 627 URL（固定 15・チーム 30・選手 582）を確認。ビルド・出力 HTML の canonical・フッター・JS バンドル内プリセットまで確認済み。**実画面が要る残り**: プリセットのタップ→比較表示、選手一覧で「jokic」ヒット、モバイルのナビ右端フェード。

| # | 項目 | 内容 | 検証 |
|---|---|---|---|
| 1 | 比較プリセット（Codex 指摘） | `/compare` の初期表示が空。`?ids=` の初期選択は実装済みなので、3〜6 組（Jokić vs SGA、得点王候補、同ポジション）を固定リンクで置く | 初回表示で比較が出る |
| 2 | canonical 2 ページ（Codex） | `/types` `/metrics` は素の `metadata` で canonical が無い。`pageMeta` に揃える | 本番 HTML に `<link rel="canonical">` |
| 3 | sitemap | `app/sitemap.ts`（選手・チーム・固定ページ・過去季）。静的エクスポートで出ることを確認 | `/sitemap.xml` 200 |
| 4 | このサイトについて | 指標解説の冒頭に 5 行（NBA.com/Stats・nba_api・毎日取得・非公式・個人運営・CSV は GitHub）。フッター 1 行 | 出典が本文に出る |
| 5 | ナビ名 | 「検索」→「比較」。モバイルナビの右端フェード（横スクロールの手がかり） | 実画面 |
| 6 | 選手一覧の検索 | 発音記号を落とさず照合しており「jokic」で Jokić が出ない（比較ページは落としている）。同じ `normalizeName` を使う | 「jokic」で当たる |
| 7 | `navigation.tsx:18` のコメント（Codex） | 旧仕様 `?phase=` → `/po` パス | — |

### 13-3. トップページの検討（§12-6 の案の有効性＋他の見せ方）

**§12-6「きょうの見どころ」＋Heartbeat 列の評価**:
- 有効な点: 見どころカードは「戻る理由」（前日から何が変わったか）を作る唯一の設計で、シーズン中は事件系（接戦・連勝・熱戦指数）が毎日変わる。順位表に直近 10 試合の Heartbeat 列を足す案は、他サイトと同じ情報だった順位表を署名のある図に変える最小改修で、評価 §4.1 の「トップに独自要素が無い」に直接効く
- 弱い点: (a) 開幕前・オフは事件プールが空で分析系 3 枚が固定になり、毎日来ても同じ画面になる。(b) 繰越直後は現季データが空なので、見どころは `allSeasons()` で「データがある最新の季」から引く必要がある（§12-11 の繰越検証で判明した空表の期間に、昨季の振り返りとして出せる）。(c) 3 枚はテキスト見出し＋小図で、初見に「このサイトの図表はこういうもの」を見せる力は 1 枚の大きな図に劣る

**入口の型の決定（2026-09-02 ふくたろう）**: 入口は「問いかける型」（推しの入力・意外の一行・日替わりクイズ）ではなく**「見せる型」（一枚の図が自分で語る）**で組む。理由: このサイトの差分は図の作り方そのもので、入口だけ別の調子（呼びかけ・文章・ゲーム）にすると着いた人が最初に見るものと中身が食い違う。sumo-data で文章の「見どころ」カードが弱かった経験と同じ。「合わなくても導入に有効なら」は採らない。クイズは独立ページなら可だがトップには置かない。

**見せる型の構成（推奨順）**:
| 案 | 内容 | 目安 | 検証 |
|---|---|---|---|
| A 30 チームの Heartbeat 壁 | 全 30 チームの 81 試合の点差バーを順位順に小さく並べ、最初の画面に置く。一行の説明（「全 30 チームの点差。上が勝ち、下が負け」）だけで問いかけはしない。自分のチームは色と略称で見つけ、クリックでチームページへ。PO 期間はブラケットがこの位置 | 半日〜1 日 | **見栄えの確認が要る**（ふくたろう）。まず現データで 30 枚を並べたモックを 1 画面作り、PC 1710px と iPhone 390px で見てから本実装に進む。確認点: 1 枚あたりの幅と高さ（PC は 6 列×5 行、スマホは 2 列×15 行が仮）、点差バーが小さくても季節の形（連勝・失速）が読めるか、順位表との重複感、ページの重さ（30 枚の SVG 追加分） |
| B 今日の一枚（図だけ） | 1 選手の Percentile バーを大きく置く。添えるのは選手名（日本語）と季だけ。**「意外の一行」は付けない**（読む人が落差に気づけばよく、こちらから言わない）。選び方は決定論（日付ハッシュ × GP 上位 100 人）。オフでも昨季データで回る | 半日 | 繰越後の状態で昨季から引けること |
| C ヘッダーの検索欄 | 比較ページの検索を、呼びかけ文なしの検索欄としてヘッダーに常設。日本語名・英字の両方で当たる（§13-1 段階 1） | 半日 | 「ヨキッチ」「jokic」で当たる |
| D 図表カタログ（§12-6 既記） | `/metrics` に実データの図を 1 枚ずつ | 1 日 | — |
| E 見どころ 3 枚（§12-6 既記） | 事件系優先。文章カードなので **A・B の後に、図の付属として** 出す（文章が主役にならない置き方） | Phase 2 | 開幕後 |

不採用: 「推しの選手は？」の呼びかけ入力（C の検索欄で足りる）、意外の一行、日替わりクイズ「この形はだれ？」、チーム格子（A が兼ねる）。

**A 案の降格（2026-09-06 ふくたろう）**: Heartbeat 壁は前提から外す（モック検証前に方向転換）。理由 3 点 — (1) Heartbeat 図自体がそれほどきれいではなく、30 枚並べると増幅される (2) シーズン序盤はバーが数本でほぼ空。公開照準が 10 月開幕なので、いちばん見せたい時期にいちばん弱い (3) 常設の壁としては単調。構造的な整理: Heartbeat・ワンマン度のような**蓄積型**の図は序盤に成立せず、パーセンタイル・四象限マップのような**位置・分布型**は 10 試合程度から成立する。トップの主役は位置・分布型から選ぶか、時期で切り替える（開幕前後=昨季の振り返り・序盤=位置型・中盤以降=蓄積型）方向で §13-3 を再検討する。「見せる型」（1 枚の図が自分で語る）の方針自体は維持。

**導入アニメーションの方針（2026-09-06 ふくたろう合意）**: dataviz 知覚容量ガイドライン（検証レポート_20260906 #7・動き知覚は同時トラッキング 3〜4 個/アイデンティティ追跡 1〜2 個。理解目的なら静的優先、アニメは engagement 向け）の 2 段判断に従い、**トップで動くのは 1 箇所だけ**にする。採用候補: (1) ヒーローの 1 枚が「描き上がる」draw-in（1 回きり・1 秒弱・ループなし。「数字が形になる」瞬間＝サイト名の実演。CSS のみ・半日） (2) 推し欄のマーカー移動（前回○→今回●。動き＝意味が一致する最も正当なアニメ。§13-4 A 実装時）。他の要素（試合結果・順位・リーダー）は静的のまま。不採用: 常時ループ・モーフィング・スクロール連動演出。`prefers-reduced-motion` 対応を必須とする。§13-3 でヒーロー候補のモックを作るとき draw-in 付きの版も並べて見比べる。

**検証**: 繰越後の状態（§12-11 の模擬ビルド手順）でトップが「2026-27 開幕前・昨季の振り返り」として成立すること。初見テストは、NBA を知っている 1 人に「このサイトは何のサイトか」を 30 秒で言ってもらう。

**ヒーロー確定・実装（2026-09-24 ふくたろう決定・同日実装）**: 入口は**「リーグの地形図」**（30チームの四象限散布図）で確定し、モック（Artifact・2025-26最終データ・PC1710/iPhone390確認）を経て本実装した。決定内容:
- **既定は「強さの地形」（ORtg×DRtg）**。トグルで「スタイルの地形」（PACE×ORtg）に切替（「スタイルになると一歩踏み込んだデータで気がついた人が見るでよい」）。切替時は点が新位置へ滑る（操作起点の動き＝上記アニメ方針の許容範囲）
- **造語は使わない**（ふくたろう指示）: 流通しているNBA用語（ORtg・DRtg・PACE等）を言い換えない。許容は「強さの地形」「スタイルの地形」程度の図の名付けまで。**象限ラベルも置かない**（無くても分かる・サイトのイメージに合わない）。軸ラベルは「攻撃レーティング (ORtg)」の併記形
- **点タップ1回目=点の近くにポップアップ**（チーム名日本語・勝敗・3指標）・**2回目=チームページへ**。図の外（最下部）の固定表示は気づかれないため不可（別案件でも指摘・QuadrantMapと同じ操作系）
- **重なる点は最小間隔まで反発させて離す**（決定論的・データ位置の僅かなずれは許容し説明行に明記）。ほぼ重なって存在が見えない点はこの図では致命的、というふくたろう判断。ラベルは点とラベルを障害物にした空き探索（右→左→下→上）
- **draw-in は初回1回だけ**（下端から定位置へ・stagger 12ms・reduced-motion対応）。モックにあった再生ボタンは本番には置かない
- **トップの並び（シーズン中）**: (1) 見出し＋「N試合消化・データ反映日」 (2) 地形図ヒーロー (3) 昨夜の結果帯（最終取得日のスコア横スクロール1行→/games） (4) 順位表 (5) リーダー3枚。リーダーの母集団は固定 GP≥30 をやめ **min(30, 最多消化チームの試合数÷2)** の暫定ゲート（bleague-data `minGp` と同型。固定30だと12月中旬までリーダーが空になる）
- bleague-data トップとの比較検証（2026-09-24）からの逆輸入: 動的GPゲート・「旗艦の図をトップに実寸・全量はカタログへリンク」型。タイル3枚（Best Record/Off/Def）は §12-9-6 の削除判断を維持（地形図が図で語る）。ミニ図ストリップ（見本帳）案は取り下げ
- 実装: `src/components/league-terrain.tsx`（client・スマホは viewBox 520×640 で文字を相対的に大きく保つ）、`home-dashboard.tsx` の RegularSeason に組み込み、`/metrics#league-terrain` に解説を追加（トップの図はカタログへリンクする §12-6 規約）
- **残課題**: (a) 繰越直後の空データ期は地形図・結果帯とも空になる（§13-3 弱点(b) の「昨季最終図フォールバック」は未実装。rollover 前に対応） (b) 旗艦選手マップ（順位表の下の実寸1枚）は未着手 (c) 初見30秒テストは本番反映後

**同日改訂（2026-09-24 本番確認後のふくたろう指摘）**: (1) 図タイトルは「リーグの地形図＋グレーの軸名」では何の図か一瞬で分からない → **タイトル自体を「強さの地形」「スタイルの地形」に**（「リーグの地形図」の名は指標解説の総称としてだけ残さず、解説タイトルも「強さの地形・スタイルの地形」に変更） (2) **PCはトグルをやめて2図並列**（players のマップ2枚と同じ Card＋`lg:grid-cols-2` の型。1枚を大きく描くと点の数・文字サイズと収まりが悪い）。**スマホはトグル切替1枚のまま**。PC/スマホの出し分けは CSS（`hidden lg:grid` / `lg:hidden`）に任せ、matchMedia 追随は削除 (3) 縦軸ラベルは左辺に沿わせて回転・横軸は下辺中央・DRtg 軸は矢印でなく「上ほど失点が少ない」と言葉で明示 (4) 試合結果帯は「見出し＋日付＋すべて見る→」の型・PCは折り返し全件表示 (5) **トップ最上部にサイトの自己紹介1行**（sumo-data の top-lead と同趣旨・§13-5 タグライン「NBAの数字を、かたちで感じる」の初掲出）——「いきなりマップ図では、独自色は出るが、わからなさが先にくる」

### 13-4. 追加候補（2026-09-02 ふくたろう提起）: 推し選手のローカル保存・図表の拡大＋画像保存

**A. 推し選手・推しチームのローカル保存**（Codex 指摘・戻る理由）
- 仕組み: 選手・チームページの見出しに ☆。`localStorage` に ID 配列（上限 5〜8）と、最後に見たときの主要値（PTS/REB/AST・percentile 8 部門・チームは W-L と NRtg）を保存。トップに「推し」欄を出し、現在値と保存値の差（「前回見てから PTS +1.2・得点 percentile 91→95」）を並べる。次の閲覧で保存値を更新
- 静的サイトでも成立する: サーバー不要・データはブラウザ内だけ。ハイドレーション後に出るので初回描画は空（レイアウトシフトを避けるため枠は先に確保する）。プライバシーは「このサイトについて」に 1 行（端末内にのみ保存・送信しない）
- 「前回比」が履歴無しで作れるのが要点。サイト側は日次スナップショットしか持たないが、差分の基準を読者のブラウザに置くことで、「あなたが最後に見てからの変化」になる。閲覧間隔が人ごとに違うので、基準日を「前回: 12/20」と併記する
- 目安: 1 日。トップの推し欄は §13-3 の「今日の一枚」と同じ Percentile バー部品を使う
- **設計方針の追加（2026-09-06 ふくたろう議論）**: (1) 主役は素の数値の差分でなく**「リーグ内の位置の変化」**（percentile の移動。「推しを☆登録するほどの人は主要スタッツの数値自体は知っている」というふくたろう指摘への答え。数値差分は添え物） (2) 保存項目は主要8部門に加えて**独自指標（ハッスル・運動量・縁の下の力持ち度）を含める**——ニュースに出ないスタッツの動きは熱心なファンにも発見になる (3) 表示は**「動いた項目だけを変化量順に2〜3件」**に絞る（保存は広く・表示は狭く。縦に嵩む問題と「何を見せるか」を同時に解く。変化ゼロの日は「大きな動きなし」1行） (4) 表示形式の第一候補は PhaseCompareBars と同型の「前回=灰・今回=カラー」2バー、トップで嵩むなら1バー＋前回位置マーカー (5) **実装は §13-3 トップページ構成と同じフェーズで**（表示先がトップの推し欄のため、トップの形が決まる前に作ると手戻り。§13-4 の優先順から A だけ §13-3 に合流）

**B. 図表の拡大表示＋画像保存**（Baseball Savant の Save as Image と同型。共有される理由）
- 仕組み: 各カードを共通の枠（`ChartFrame`）で包み、クリックで `<dialog>` に拡大。「画像を保存」で PNG にし、下端に「選手名（日本語）・季・データ反映日・nba-data.pages.dev」を焼き込む。**出典入りの画像が SNS に流れることが共有の本体**（国会のしごとの切り抜き耐性と同じ考え。画像単体で出所が分かる）
- 実装上の分岐: 純 SVG の図（Heartbeat・四象限・ローレンツ・レーダー・ショットチャート・使われ方マップの 6 部品）は SVG 直列化 → canvas → PNG で依存追加なし。Percentile バー・ワッフル・運動量など HTML で組んだカードは `html-to-image` 1 依存で DOM → PNG。比較ページのレーダーは Recharts（SVG）なので前者。OG 画像の `ImageResponse` はビルド時生成なので流用しない（選手×カード数のファイル増になる）
- スマホ: `navigator.share({files})` が使える端末では X・LINE へ直接。使えなければダウンロード。Cloudflare Pages の通常ページなのでダウンロードは普通に動く（Artifact のような制限はない）
- 暗背景を canvas に先に塗る（SVG は透明）。フォントは端末のシステムフォントで描かれるので、日本語名は保存側の環境に依存する（Savant も同じ）
- 目安: 枠と保存の共通部品で 1 日、既存カード約 12 枚への適用で半日。§13-3 の「今日の一枚」に先に付けて反応を見る
- **実装済み（2026-09-06）**: `ChartFrame`（`src/components/chart-frame.tsx`）で17枚に適用。設計変更2点: (1) SVG/HTML の2経路でなく、モーダルの DOM 全体（ヘッダー・フッターのテキスト込み）を `html-to-image` で1発 PNG 化する1経路に統一（表示と保存が必ず一致、依存1つ・保存時のみ動的 import） (2) 開き方は全図とも図の下の「画像保存」ボタンに統一（当初は操作のない図だけ全面クリックにしたが、レーダー等もホバー操作がありスマホはホバー自体が無く気づけない、モーダルは拡大でもない、というふくたろう実機指摘で表記ごと収斂。iOS Safari は遅延ダウンロードを静かに無視するため、スマホは共有シート・PNG はモーダルを開いた時点で事前生成）。内訳: 選手ページ6・チーム5・リーダーズ2・選手一覧2・比較レーダー2。データ反映日は現行シーズンのみ・PO の図は PO 最終戦日。navigator.share は未実装（スマホ実機の反応を見てから）

**順序**: B を A より先に。B は既存の図表がそのまま資産になり、A はトップの推し欄という新しい面を要る。どちらも §13-2 の小修正と §13-1 段階 1（日本語名）を終えてから

**C. CSV 配布**（2026-09-06 追記）: 当初は「CSV は GitHub」で出す構想（§13-2 の表・行4）だったが、リポジトリが非公開のため「このサイトについて」から GitHub リンク行を削除した（`3ce0a29`）ところで停止中。再開時はサイト自体から配る形（例: ビルド時に `data/*.csv` を `public/data/` へコピーし、データダウンロードのページか「このサイトについて」に一覧を置く。Cloudflare Pages の静的ファイルなのでサーバー不要）を検討する。出典表記・利用条件の1行を添えること
- **実装済み（2026-09-06）**: 最小形で実装。`prebuild` で `data/` 直下の CSV 23本（約750KB）を `public/data/` へコピー（`rm -f` 付きで rollover 後の po_系削除にも追随）。専用ページは作らず `/metrics` の「このサイトについて」カードに折りたたみ一覧＋出典・利用条件1行。一覧はビルド時に `data/` を読んで生成するのでコピー集合と常に一致。`boxscores/`・`shots/`・過去季 `data/2025-26/` は対象外（需要「簡単なことなら配布してもよい程度」＝ふくたろう評価に合わせ最小で止めた）

**優先順（2026-09-06 ふくたろう決定）**: §13-3 トップページ構成より先に、この §13-4 の3件（C. CSV 配布・B. 図表の画像保存・A. 推し選手のローカル保存）を進める。次セッションはここから着手

### 13-5. サイト名（決定・実装済み 2026-09-03）

「NBA Data」は説明であって名前ではなく、検索で唯一にならない。固有名＋タグラインに分ける。第一候補「**スタッツのかたち**」、タグライン「NBAの数字を、かたちで感じる」（2026-09-02 ふくたろう「少し考える」）。条件: 固有名に NBA を含めない（商標）、6〜8 文字、「NBA」「スタッツ」は説明句と title の説明部に置く。変更範囲は `SITE_NAME` 定数・OG 画像 10 ファイルの文字列・ナビ・README で半日。画像保存（§13-4 B）と OG に名前が焼き込まれる前に決める。URL（独自ドメイン）は名前が決まってから。

**決定（2026-09-03 ふくたろう）**: サイト名「スタッツのかたち」で確定・実装した。「NBA」はシーズン副記として残す（名前に融合させない。ヘッダーはロゴ「スタッツのかたち」＋副記「NBA 2025-26」。シーズン年を名前に焼き込むと繰越のたびに名前が変わるため）。OG 画像内は英数字のみ決定（§、og.tsx）に従いローマ字「Stats no Katachi」（`SITE_NAME_EN`）。metrics/types の kicker「NBA Data」は左上ブランドと重複していたため削除。タグラインの掲出と独自ドメインは未着手。

### 13-6. ナビゲーションとブランドの役割分担・作業工程（2026-09-03 整理）

**決定（2026-09-03 ふくたろう）**:
- ヘッダーは当面**ロゴマークのみ**（PC も文字サイト名を出さない）。日本語サイト名のシステムフォント太字は「黒バック白ボールドがサイトに合わない・窮屈」で、置くならロゴタイプとして精緻にデザインしてから。**ロゴマーク・ロゴタイプはふくたろうさんが新たに検討**（Claude は決定後の実装のみ。現行マークは仮）
- ヘッダーのアイコンナビは**減らさず現状維持**。「わからないなりのアイコンのクールさも好き。タップしてみようかなという気になる」— アイコンの不明瞭さは欠点であり誘引でもある、と割り切る
- **全ページの一覧はフッターが持つ**（同日実装: navItems を共有化しフッターに 9 リンク＋サイト名（トップへのリンク）＋出典行）
- **トップページの構成は §13-3 の別フェーズで検討**。「今日の一枚」は再検討対象（ふくたろう「クールなデータ表示サイトのイメージを安易な言葉カードで崩したくない」。図だけ案でも採否は保留）。Heartbeat 壁もモックで見てから。主要ページへの本文内誘導（ナビ紹介）もこのフェーズで扱う
- 一般公開の照準は **10 月の開幕時期**

**作業工程（開幕公開まで・順序どおり）**:
1. ✅ §13-5 サイト名・§13-2 小修正 7 件（2026-09-03 本番反映済み）
2. ✅ ヘッダー当面マークのみ・フッターナビ整備（2026-09-03）
3. §13-1 日本語名 段階 1（title/description・比較検索）← 次の実装
4. ブランド確定待ち: ロゴマーク・ロゴタイプ（ふくたろうさん検討）→ 決定後にヘッダー・フッター・OG・共有画像へ反映
5. §13-3 トップページ構成（Heartbeat 壁モック→PC/スマホ見栄え確認→採否。今日の一枚の再検討・本文内誘導もここ）
6. §13-4 B 図表の拡大＋画像保存 → A 推し選手のローカル保存
7. 開幕前: rollover.sh 実行と §12-11 繰越検証・OGP 実機確認（トラッカー既載）
8. 開幕後: GA4 実績を見て Phase 2（見どころ等・§12-6〜12-8）

### 13-7. マップの対象絞り込み（2026-09-06 実装）

ドット密度の指摘（ふくたろう・Codex レビュー由来）への対応。MPG 一律で切る案は「STL 上位のサイブル（MPG16.0・チーム内 MP13 位）らがリストに居るのに図から消える」問題があり（現行 GP40・MPG25 でも既に消えていた。チーム内出場時間上位 N 方式だと怪我明けのウェンバンヤマまで落ちる）、図の性格に合わせて基準を分けた。

- **リーダーの図（RS/PO 共通）**: 対象＝そのリストの上位 20 人の和集合。「図に載る人＝リストに載っている人」で §11 の「図はリストのスタッツだけで構成」の延長。守備=STL∪BLK（RS40・PO36 点）、USG%×TS%（RS40・PO40 点）。全員がどちらかの軸の上位なので左下象限はほぼ空く（象限ラベルは実画面確認のうえ現状維持で確定。空の象限にもラベルがあることで「ここに誰も居ない」が読める。2026-09-06 ふくたろう判断）
- **選手一覧 USG%×TS%**: 各チーム出場時間上位 5 人（RS150・PO80 点。旧 452 点）。点数が 30×N で安定し全チームが登場する
- **選手一覧シューター**: 3PA 4.0 本以上（RS162・PO64 点。旧 3PA1.0 で 382 点）。横軸そのものによる資格制なので分布は歪まない
- 選手一覧の両図にも `labelTop={5}`（両軸上位 5 人の常時名前ラベル）を追加
- 実装メモ: `"use client"` モジュールから定数（TOP_N）をサーバーへ import すると client reference 化して値が取れず図が 0 人になる → `leaders/constants.ts` に分離

### 13-8. チームページ: 冒頭カードのスマホ2行化＋守備4ファクター図（2026-09-06 実装）

ふくたろう依頼2件。①スマホで ORtg〜PIE の5カードが縦積みで縦幅を取りすぎ → `grid-cols-3 sm:grid-cols-5`（3+2 の2行）、正式名の sub はスマホでは非表示。②図表が Heartbeat 以外オフェンス寄り（ワンマン度・ボール支配）→ 守備図を追加。

- **守備4ファクター**（Dean Oliver の Four Factors 守備版）: 被eFG%（シュート抑止）・奪TOV率（ボール奪取）・DRB%（リバウンド）・被FTレート（ファウル抑制）の4部門をリーグ30チーム中の順位バーで表示（左端=1位）。総合守備力（DRtg）は冒頭カードにあるので、こちらは「何で守るか」のスタイルを見る図
- 選定経緯: 候補は 30チーム散布図（2軸のみ・QuadrantMap のチーム対応改修が要る）と守備ハッスル帯（ロスター内訳でチームの特徴ではない）。4本の順位バーが「守り方のタイプ」を最も直接に表すため採用（2026-09-06 ふくたろう選択）
...
### 13-10. 過去シーズンの扱い（2026-09-22 ふくたろう指示・方針確定）

新シーズンへ向けたトップページ（§13-3）等の残課題と過去季対応は**一括して進める**。方針は次の3点で、これが nba-data の過去季設計の正（§12-3〜12-5 と整合。矛盾したらこの節が勝つ）:

1. **過去季（2025-26）は「シーズン全体として独立して存在する」軽量アーカイブ**: 分量的に負担にならないページだけを残す。当季と対称な全ページ並行閲覧（bleague-data の一覧系 `?season=` 全面適用）は**やらない**
2. **選手ページはスタッツ表の積み重ね**: 過去シーズン分が現シーズンの下に載る形。§12-3 で設計済み・Phase 1 (c) で実装済み（Stats 表が `allSeasons()` を回す）。ロールオーバー直後の表示実確認（`?season=2025-26` で RS 上）は §12-8 Phase 1 の宿題のまま
3. **選手ページは「シーズンごとのページ」ではなく選手主体の構成とし、過去季の feel-viz も見られる形で残す**（2026-09-22 ふくたろう決定・同日追補2）: 選手個別の feel-viz が nba-data のメイン部分のため、廃止・積み重ね表への統合はしない。そのうえで**内容を圧縮する方法を検討する**——§12-11 の Cloudflare ファイル数上限（過去季3つ目・2029年春で 20,000 超）への対策。検討案:
   - **有力（ふくたろう提起）**: feel-viz 部分は初期表示＝今シーズン、**ページ内のシーズンセレクタで過去シーズン分に切り替える**。選手1人＝1ページ（選手主体）になり、§12-3「図表は選択中シーズンのみ・セレクタで切替」と同方向。過去季の別URL（`/players/[id]/2025-26`）と季数に比例するファイル数増加も解消し得る
   - 既出の案（§12-11）: (a) 過去季ページをローテ選手（GP≥MIN_GP）に絞る (b) 最古の季を落とす (c) 1ページあたりの出力削減（実測: HTML 349KB/人＋RSC `.txt`＋セグメント計約9ファイル/人）

**bleague-data を先例にしない**（2026-09-22 ふくたろう）: bleague は feel-viz 実装事例としての位置づけで、手をかけない。bleague 側の複数シーズン実装（一覧系 `?season=`・リンク伝播）を nba-data に逆輸入する提案は却下済み。

未確定（着手時に判断）:
- 「負担にならないページ」としてどれを残すか（候補: 順位表・リーダーあたり。トップページ再設計 §13-3 と同時に選定）
- 選手ページの構成方式（上記3のページ内シーズン切替を採るか、別URL維持＋圧縮か）。ページ内切替を採る場合は、過去季データの持ち方（ページ埋め込みだと1ファイルが肥大／季別JSON分割だとファイル数は減らない点の実測比較）と、過去季表示への直リンク・共有の扱いを静的エクスポートの制約内で設計する
- 過去季ページ内リンクが全て当季へ出る点（2026-09-22 コード実測: 季を保つ導線は SeasonSwitch のみ）を「独立アーカイブ」の枠内でどう再設計するか（ページ内切替を採れば選手ページについては消える論点）

**次セッションの作業指示（2026-09-24 ふくたろう。上の未確定への回答を含む）**:
1. **シーズンUIの混乱防止**: シーズンを切り替えたら**そのシーズン内で横移動するナビ**にする（上記「過去季リンクが全て当季へ出る」問題への方向決定。§13-10-1 の「独立アーカイブ」の枠内で、残すページ間は季を保って行き来できる形）
2. **2025-26 で残すコンテンツの検証**: どのページを残すか＋有効な圧縮方法（§12-11 のファイル数上限対策と合わせて）
3. **選手ページの過去スタッツ**: 今シーズンのスタッツ表の下に**過去シーズンを畳んで**載せる。**キャリア通算（トータル）行は不採用**（過去が1シーズン分しか無いため）
4. **選手ページの図表**: 過去シーズンへの**切替表示**を可能にする（§13-10-3 の有力案どおり）。加えて**重ねて比較表示**（今季×昨季を同じ図に）が成立するか、データ量（ページ埋め込みのサイズ増）に問題が無いかを検証
5. **別件・有力**: 推し選手のローカル保存＋保存時データと現在データの比較表示（§13-4 A。設計方針は §13-4 A の追加5点が既定）

### 13-11. 地形図・マップの個別ページ展開（2026-09-25 ふくたろう指示・実装）

**チームページ**（`59bd345`・`c99f37e`）: トップの「強さの地形」「スタイルの地形」を各チームページの冒頭カード直下に掲載。当該チーム以外は40%・当該チームは点/略号を1.35倍＋ORtg/DRtg/PACE の数値箱を常時表示（箱は上下左右4候補から自分の点・略号を覆わず他の点を覆う数が最少の位置）。データ構築は `getTerrainTeams()` に共通化。副作用: React の transition/transitionDelay 併用警告のコードフレーム描画が Next 16 の next-code-frame 多バイト文字パニック（vercel/next.js#98178・#98714、修正PR未マージ）で dev サーバーを落とした → 警告の発生源を消して回避。Next.js 更新は修正マージ後（決定ログ 2026-09-25）

**選手ページ・段階1**: 「使われ方 × 効率」（USG% × TS%）を League Percentile の直下に。背景＝GP≥`MIN_GP` の全員（2025-26最終で452人）・当該選手を強調（点を大きく・名前と値の箱を常時・他は40%）。QuadrantMap に `highlight` を追加して流用（/players・/leaders の図は無変更）
- **データは共有JSON**（`src/app/data/player-map-usg-ts.json/route.ts`・Route Handler の静的書き出し・シーズン別・約35KB）。ページ埋め込みは HTML＋RSC payload の二重で 583ページ約+120MB になるため不採用
- **条件外の扱い**: GP<20 は League Percentile と同じ基準で図を出さない（新しい規則を増やさない）。移籍先で母集団に居ない場合も出さない
- **4マップの取捨**（検証: `_ai_workspace/reports/2026-09-25-選手ページへの選手マップ掲載の検証.md`）: USG%×TS% は相関 +0.02・四象限ほぼ均等で全員に有効。3PA×3P% は 3PA≥4 の該当者（162人）にのみ有効 → 段階2として該当者だけに追加する案だったが、**保留**（2026-09-25 ふくたろう）: シューター特化の図なので「該当者にだけ図が1枚増える」以外の掲載方法も含めて検討が要る。需要はある（八村がシューター特性が高い例）ので却下はしない。参考: 八村は 2025-26 最終で 3PA/G 3.9・3P% 44.3% と現行閾値 4.0 の境界上（閾値 3.5 なら 195人中 3P% 4位、3.0 なら 236人中 5位）。2026-27 は試投増が見込まれるため閾値変更は前提にしない（ふくたろう）。STL×BLK は STL≤1.0 かつ BLK≤0.5 に61%が固まり個人ページでは弱い → 見送り（/leaders の型分けとしては維持）
- 残課題: §13-3 残課題(b) 旗艦選手マップ（トップ）は同じ JSON と部品で作れる
