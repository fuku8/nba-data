...
- **実装済み（2026-09-06）**: `ChartFrame`（`src/components/chart-frame.tsx`）で17枚に適用。設計変更2点: (1) SVG/HTML の2経路でなく、モーダルの DOM 全体（ヘッダー・フッターのテキスト込み）を `html-to-image` で1発 PNG 化する1経路に統一（表示と保存が必ず一致、依存1つ・保存時のみ動的 import） (2) 開き方を図で分ける — 操作のない図は全面クリック、ドット操作のある図（四象限マップ・使われ方/シューターマップ・Heartbeat）は図の下の「⤢ 拡大・画像保存」ボタン（ふくたろう指摘）。内訳: 選手ページ6・チーム5・リーダーズ2・選手一覧2・比較レーダー2。データ反映日は現行シーズンのみ・PO の図は PO 最終戦日。navigator.share は未実装（スマホ実機の反応を見てから）
