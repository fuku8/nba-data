...
2. 選手ページ見出しを日本語名＋英語名（副）、OG 画像も同じ。**比較ページの選択バッジ・凡例・表の列ヘッダーもここで日本語化**（プリセットは日本語・結果は英語という段階1の不一致を解消。同時表示4人までなので幅の論点が小さく先行しやすい。2026-09-03 ふくたろう了承） → 検証: `/players/203999` と OG の実表示
