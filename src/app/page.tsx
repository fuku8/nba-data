...
function LeaderCard({
  ...
  title: string;
  players: { name: string; team: string; value: number }[];
}) {
  return (
    <Card>
...
export default function HomePage() {
  ...
  const ptsLeaders = [...players]
    ...
    .slice(0, 5)
    .map((p) => ({ name: p.player, team: p.team, value: p.pts }));

  const rebLeaders = [...players]
    .sort((a, b) => b.trb - a.trb)
    .slice(0, 5)
    .map((p) => ({ name: p.player, team: p.team, value: p.trb }));

  const astLeaders = [...players]
    .sort((a, b) => b.ast - a.ast)
    .slice(0, 5)
    .map((p) => ({ name: p.player, team: p.team, value: p.ast }));

  const eastTop = standings
    .filter((s) => s.conference === "East")
    .sort((a, b) => b.winPct - a.winPct)
    .slice(0, 8);

  const westTop = standings
    ...
    <div className="space-y-8">
      ...
      <div className="grid gap-4 sm:grid-cols-3">
        {bestOff && (
          <Card>
            <CardHeader className="pb-2">
              <CardTitle className="text-sm text-muted-foreground">Best Offense</CardTitle>
            </CardHeader>
            <CardContent>
        ...
        {bestNet && (
          <Card>
            <CardHeader className="pb-2">
              <CardTitle className="text-sm text-muted-foreground">Best Net Rating</CardTitle>
            </CardHeader>
            <CardContent>
      ...
      <div>
        <div className="flex items-center justify-between mb-4">
          ...
          <Link href="/leaders" className="text-sm text-muted-foreground hover:underline">
            すべて見る →
          </Link>
        </div>
        <div className="grid gap-4 sm:grid-cols-3">
      ...
      <div>
        ...
        <div className="grid gap-6 lg:grid-cols-2">
          {[
            { title: "Eastern Conference", teams: eastTop },
            { title: "Western Conference", teams: westTop },
          ].map(({ title, teams }) => (
            <Card key={title}>
              <CardHeader className="pb-3">
