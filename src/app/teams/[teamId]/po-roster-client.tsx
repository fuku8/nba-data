    ...
    <div className="rounded-md border overflow-x-auto">
      <table className="w-full text-sm">
        <thead className="border-b bg-muted/50">
          <tr>
            <th className="text-left py-2 px-3 font-medium sticky left-0 z-10 bg-muted/50 min-w-[12em]">選手</th>
            {COLS.map((col) => (
        ...
        <tbody>
          {sorted.map((p) => (
            <tr key={`${p.player}-${p.team}`} className="border-b last:border-0 hover:bg-muted/30 transition-colors">
              {/* フル名は切り詰めず「・」「＝」で折り返す。min-w が無いと w-full の表が名前列を潰す */}
              <td className="py-2 px-3 font-medium sticky left-0 z-10 bg-background min-w-[12em] max-w-[12em] sm:max-w-none leading-snug">
                <Link href={`/players/${p.playerId}`} className="block hover:underline"><SegmentedName name={p.player} /></Link>
              </td>
              {COLS.map((col) => {
