...
// 先頭から段階的に薄くする（帯と凡例で共通）
const segOpacity = (i: number) => Math.max(0.2, 1 - i * 0.11);
  ...
  return (
    <div className="space-y-3">
      ...
      <div className="flex flex-wrap gap-x-4 gap-y-1 text-xs text-muted-foreground">
        {segments.map((s, i) => (
          <span key={`${i}-${s.name}`} className="flex items-center gap-1.5">
            <span
              className="h-2.5 w-2.5 rounded-[2px] inline-block"
              style={{ backgroundColor: color, opacity: segOpacity(i) }}
            />
